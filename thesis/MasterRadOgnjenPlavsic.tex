...
\begin{tikzpicture}
  ...
  \end{class}
\end{tikzpicture}
  \caption{Odnos klasa \texttt{DiagnosticConsumer}, \texttt{TextDiagnosticPrinter} i \texttt{AutoFixDiagnosticConsumer}.}
...
Alat \textit{lit} slu\v{z}i za izvr\v{s}avanje testova i testnih paketa (eng.~\textit{test suites}) u okviru kompajlerske infrastrukture LLVM. Alat takodje sumira rezultate i generi\v{s}e informacije o gre\v{s}kama u okviru testova. Testovi implementirani u okviru alate \textit{AutoFix}
...
Detaljnim testiranjem upotrebom alata \textit{lit} i \textit{FileCheck} iz kompajlerske infrastrukture LLVM obezbedjen je i visok
