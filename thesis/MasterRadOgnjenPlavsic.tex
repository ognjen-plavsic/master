...
\begin{figure}[!ht]
  ...
  \caption{Primer reprezentacije apstraktnog sintaksnog stabla generisanog komandom: \texttt{clang -Xclang -ast-dump hello.c}. Apstraktno sintaksno stablo prikazano na slici prezentuje k\^{o}d iz listinga \ref{lst:label4}.}
  \label{fig:ASTSlika}
\end{figure}
...
\begin{enumerate}
  \item Metode \texttt{TraverseDecl(Decl *x)}, \texttt{TraverseStmt(Stmt *x)} i \texttt{TraverseType\-(QualType x)} obavljaju prvi zadatak time \v{s}to implementiraju obilazak, redom, deklaracija, izraza i tipova u okviru apstraktnog sintaksnog stabla. Ovo su ulazne tačke za obilazak apstraktnog sintaksnog stabla sa korenom u čvoru x. Ove metode pozivaju metod \\ \texttt{TraverseFoo(Foo *x)}, gde je \texttt{Foo} dinamički tip od \texttt{*x}, koji poziva metod \\ \texttt{WalkUpFromFoo(x)}, a zatim rekurzivno posećuje decu čvora \texttt{x}. \\
...
\item Metod \texttt{WalkUpFromFoo(Foo *x)} izvršava drugi zadatak, odnosno obilazi klasnu hijerarhiju za \v{c}vor \texttt{x}. Ovaj metod ne pokušava odmah da poseti decu čvora \texttt{x}, umesto toga prvo zove \texttt{WalkUpFromBar(x)}, gde je \texttt{Bar} direktna nadklasa klase \texttt{Foo}, i tek onda zove \texttt{VisitFoo(x)}. \\
Na primer, \texttt{WalkUpFromCXXRecordDecl(CXXRecordDecl *x)} poziva \\ \texttt{WalkUpFromRecordDecl(x)} i \texttt{VisitCXXRecordDecl(x)}.
\item Metod \texttt{VisitFoo(Foo *x)} izvršava tre\'{c}i zadatak time \v{s}to analizira \v{c}vor \texttt{x} tipa \texttt{Foo}. \\Na primer, metod \texttt{VisitCXXRecordDecl(CXXRecordDecl *x)} mo\v{z}e biti predefinisan kako bi se analizirao \v{c}vor \texttt{x} tipa \texttt{CXXRecordDecl}.
\end{enumerate}
...
Na listingu \ref{lst:labelConsumer} prikazana je implementacija klase \texttt{AutoFixConsumer}. U okviru ove klase kreiran je upariva\v{c} 
\texttt{A7\_2\_3\_Matcher} (linija 11) i objekat klase povratnog poziva (linija 15). Kori\v{s}\'{c}ena klasa povratnog poziva \texttt{A7\_2\_3} prikazana je na listingu \ref{lst:MatcherList} i dostupna je kroz zaglavlje \texttt{AutoFixMatchers.h}. Upariva\v{c} i objekat klase povratnog poziva se zatim registruju za obilazak apstraktnog sintaksnog stabla koji izvr\v{s}ava objekat klase \texttt{MatchFinder} (linija 18). Nakon registrovanja poziva se metod \texttt{matchAST} kojim se registrovani upariva\v{c} pokre\'{c}e nad asptraktnim sintaksnim stablom (linija 21).
...
public:
  ...
  virtual void HandleTranslationUnit(clang::ASTContext &Context) {
    ...
    MatchFinder::MatchCallback *Callback = new A7_2_3(Context);
...
Klasa \texttt{AutoFixAction} koristi klasu \texttt{AutoFixConstumer} sa listinga \ref{lst:labelConsumer} dostupnu kroz zaglavlje \texttt{AutoFixConsumer.h}
...
U svrhu verifikacije robusnosti i kvaliteta alata \textit{AutoFix}, izvr\v{s}ena je analiza nad delovima projekta \textit{Automotive Grade Linux (AGL)} \cite{AGL}. Projekat \textit{AGL} je izabran na osnovu svoje relevantnosti u automobilskoj industriji. \par
