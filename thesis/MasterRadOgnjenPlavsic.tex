...
AST-upariva\v{c} (ili samo upariva\v{c}) je objekat \v{s}ablonske klase \texttt{Matcher}. Upariva\v{c}i slu\v{z}e za uparivanje \footnote{\textit{Uparivanje} \v{c}vorova koji \v{c}ine deo apstraktnog sintaksnog stabla zna\v{c}i prona\'{c}i ih u apstraktnom sintaksnom stablu i sa\v{c}uvati ih u odredjene strukture podataka kako bi se kasnije mogli analizirati.} dela apstraktnog sintaksnog stabla koji je opisan prilikom kreiranja upariva\v{c}a. Upariva\v{c}i se kreiraju izrazima za uparivanje (eng.~\textit{match expressions}), odnosno izrazima jezika specijalne namene implementiranog u okviru biblioteke AST-upariva\v{c}a (eng.~\textit{LibASTMatchers}). Ovaj jezik pru\v{z}a mogu\'{c}nost kreiranja predikata nad apstraktnim sintaksnim stablom. 
...
\end{center}
\caption{Klase koje nasleđuju klasu \texttt{ASTConsumer}. Slika je preuzeta sa linka \url{https://clang.llvm.org/doxygen/classclang_1_1ASTConsumer.html}.}
...
koja omogu\'{c}ava ispisivanje opcija koje se mogu zadati kompajleru i \texttt{PreprocessorFrontendAction} koja omogu\'{c}ava izvr\v{s}avanje akcija vezanih za pretprocesiranje izvornog koda. Međutim, naj\v{c}es\'{c}a upotreba ove klase vezana je za akcije koje se izvr\v{s}avaju nad apstraktnim sintaksnim stablom. U ovu svrhu koristi se apstarktna klasa \texttt{ASTFrontendAction} koja je direktna potklasa klase \texttt{FrontendAction}. \par
...
\end{center}
\caption{Klase koje nasleđuju klasu \texttt{ASTFrontendAction}. Slika je preuzeta sa linka \url{https://clang.llvm.org/doxygen/classclang_1_1ASTFrontendAction.html}.}
\label{fig:ASTAction}
\end{figure}
