...
U ovom poglavlju opisane su biblioteke i klase kompilatorske infrastrukture LLVM koje su kori\v{s}\'{c}ene za implementaciju alata za stati\v{c}ku analizu \textit{AutoHint}. Biblioteke su opisivane ukoliko su u celosti
...
S obzirom na to da se alat \textit{AutoHint} zasniva na analizi apstraktnog sintaksi\v{c}kog stabla, u ovom poglavlju opisana je biblioteka \texttt{clangAST} koja implementira osnovne
...
Na listingu \ref{lst:labelConsumer} prikazana je implementacija klase \texttt{AutoHintConsumer}. U okviru metode \texttt{HandleTranslationUnit}
poziva se funkcija za kreiranje i pokretanje upariva\v{c}a \texttt{matchASTExample} (linija 9). 

\begin{lstlisting}[style=customc,  caption={Implementacija klase \texttt{AutoHintConsumer}. Funkcija \texttt{matchASTExample} prikazana je na listingu \ref{lst:MatcherList} i dostupna je kroz zaglavlje \texttt{AutoHintMatchers.h}.}, label=lst:labelConsumer]
#include "clang/AST/ASTConsumer.h"
#include "AutoHintMatchers.h"

class AutoHintConsumer : public clang::ASTConsumer {
public:
  explicit AutoHintConsumer(ASTContext *Context) : Context(Context) {}
...
Na listingu \ref{lst:ASTAction} prikazana je implementacija klase \texttt{AutoHintAction} koja izvr\v{s}ava akcije nad apstraktnim sintaksnim stablom.
Klasa \texttt{AutoHintAction} u okviru metode \texttt{CreateASTConsumer} (linija 6) kreira pokaziva\v{c} na objekat klase \texttt{AutoHintConsumer}.

\begin{lstlisting}[style=customc,  caption={Implementacija klase \texttt{AutoHintAction}. Klasa \texttt{AutoHintConstumer} prikazana je na listingu \ref{lst:labelConsumer} i dostupna je kroz zaglavlje \texttt{AutoHintConsumer.h}.}, label=lst:ASTAction]
#include "AutoHintConsumer.h"

class AutoHintAction : public clang::ASTFrontendAction {
public:
  virtual std::unique_ptr<clang::ASTConsumer>
  CreateASTConsumer(clang::CompilerInstance &Compiler, 
                    llvm::StringRef InFile) {
    return std::make_unique<AutoHintConsumer>(
...
int main(int argc, char **argv) {
  if (argc > 1) {
    clang::tooling::runToolOnCode(
                    std::make_unique<AutoHintAction>(),
                    argv[1]);
  }
}
\end{lstlisting}
...
Kompilatorska infrastruktura LLVM sadr\v{z}i alate koji se mogu koristiti u svrhu pisanja i pokretanja testova. U svrhu testiranja alata \textit{AutoHint}
kori\v{s}\'{c}eni su alati \textit{lit} i \textit{FileCheck}. Ovi alati imaju raznoliku upotrebu i \v{s}irok spektar opcija. U nastavku \'{c}e biti opisana samo svojstva ovih alata relevantna za testiranje alata \textit{AutoHint}. \par
\begin{description}
...
\chapter{Alat AutoHint}
\label{chp:autohint}

Alat \textit{AutoHint} predstavlja alat za stati\v{c}ku analizu izvornog koda napisanog u jeziku C++14. Alat prijavljuje upozorenja
...
Alat \textit{AutoHint} se pokre\'{c}e komandom:
\\ \\
 \indent \indent \texttt{auto-hint [options] <source0> [... <sourceN>]} \\

\noindent Argument \texttt{options} ozna\v{c}ava opcije koje se mogu proslediti alatu \textit{AutoHint},
...
Mogu\'{c}e opcije su:
  \begin{description}
    ...
    analizu sa svim implementiranim pravilima u okviru alata. Ukoliko se ova opcija ne navede prilikom pokretanja, \textit{AutoHint} \'{c}e ovo protuma\v{c}iti kao da je navedena opcija \texttt{--rules=""}, odnosno ne\'{c}e se izvr\v{s}iti analiza ni za jedno pravilo.
    Primer kori\v{s}\'{c}enja ove opcije: \newline\newline
    \texttt{auto-hint ./AutoHintTest.cpp --rules="A7\_2\_3, A7\_1\_6"} \\
    \item \texttt{--help}: Ovom opcijom se ispisuje uputstvo za upotrebu alata.
  \end{description}

Pored opcija koje su definisane u okviru alata \textit{AutoHint}, prilikom pokretanja alata mogu se dodati i opcije koje se prosleđuju kompilatoru \textit{Clang}. Na primer, mo\v{z}e da bude korisno da se navede opcija \texttt{-Wno-everything} kako bi se prijavljivala isklju\v{c}ivo upozorenja generisana alatom \textit{AutoHint} i ignorisala sva upozorenja koja generi\v{s}e kompilator \textit{Clang} tokom kompilacije. Ovo se mo\v{z}e posti\'{c}i komandom: \\ \\
\texttt{./auto-hint --rules="all" test.cpp --extra-arg="-Wno-everything" --}
...
\\
\indent
Sva implementirana pravila u okviru alata \textit{AutoHint} spadaju, prema klasifikaciji iz sekcije \ref{sec:klasifikacija}, u sledeće kategorije:
\begin{enumerate}
...
Na listingu \ref{lst:label13} prikazan je k\^{o}d nad kojim \'{c}e biti ilustrovana podr\v{s}ka pravilu \textbf{A8-5-3} u okviru alata \textit{AutoHint}. 
...
Na slici \ref{fig:A8-5-3} prikazan je ispis alata \textit{AutoHint} za k\^{o}d sa listinga \ref{lst:label13}. Alat u oba slu\v{c}aja predla\v{z}e
da promenljiva bude deklarisana koriste\'{c}i simbol \texttt{=}.

\begin{lstlisting}[style=customc, caption={K\^{o}d nad kojim je demonstrirana podr\v{s}ka pravilu \textbf{A8-5-3} u okviru alata \textit{AutoHint}. Ispis alata \textit{AutoHint} nakon pokretanja nad ovim kodom prikazan je na slici \ref{fig:A8-5-3}.}, label=lst:label13]
...
Na listingu \ref{lst:label14} prikazan je k\^{o}d nad kojim je demonstrirana podr\v{s}ka pravilu \textbf{A8-5-2} u okviru alata \textit{AutoHint}.
...
Ispis alata \textit{AutoHint} kada se pokrene nad fajlom sa kodom iz listinga \ref{lst:label14} prikazan je na slici \ref{fig:A8-5-2}. \\

\begin{lstlisting}[style=customc, caption={K\^{o}d nad kojim je demonstrirana podr\v{s}ka pravilu \textbf{A8-5-2} u okviru alata \textit{AutoHint}.}, label=lst:label14]
#include <cstdint>
#include <initializer_list>
...
Oba nedostatka mogu se zaobi\'{c}i kori\v{s}\'{c}enjem specifikatora \texttt{using}.  Ispis alata \textit{AutoHint} kada se pokrene nad fajlom sa kodom iz listinga \ref{lst:typedef} prikazan je na slici \ref{fig:A7-1-6}. Alat \textit{AutoHint} od izraza za kreiranje pseudonima za tip kori\v{s}\'{c}enjem
...
Ispis alata \textit{AutoHint} kada se pokrene nad fajlom sa kodom iz listinga \ref{lst:enum} prikazan je na slici \ref{fig:A7-2-3}. 
...
Alat \textit{AutoHint} implementiran je u okviru projekta \texttt{clang-tools-extra}, potprojekta kompilatorske infrastrukture LLVM. Projekat \texttt{clang-tools-extra} sadr\v{z}i alate implementirane interfejsima za alate kompilatora \textit{Clang} (eng.~\textit{Clang’s tooling APIs}).
Alat \textit{AutoHint} je podeljen na \v{c}etiri jedinice prevođenja: \texttt{AutoHint.cpp}, \texttt{AutoHint\-Matchers.cpp}, \texttt{AutoHintDiagnosticConsumer.cpp} i \texttt{AutoHintHelper.cpp}.

\subsection{AutoHintMatchers.cpp}
Alat \textit{AutoHint} koristi biblioteku \texttt{LibAstMatchers} za analizu i obradu apstraktnog sintaksi\v{c}kog stabla kompilatora \textit{Clang}. 
\textit{AutoHint} defini\v{s}e upariva\v{c}e za uparivanje osnovnih konstrukta na koje se pravilo odnosi i za su\v{z}avanje pretrage apstraktnog sintaksi\v{c}kog stabla. Na primer, ukoliko je pravilo vezano za enumeratore (nabrajanja), upariva\v{c} koji odgovara ovom pravilu \'{c}e upariti sve deklaracije enumeratora iz apstraktnog sintaksi\v{c}kog stabla ali \'{c}e i suziti pretragu tako \v{s}to \'{c}e uparivati samo deklaracije koje nisu implicitne. Ovo se mo\v{z}e posti\'{c}i izrazom za uparivanje \texttt{enumDecl(unless(isImplicit()))}. Svakom pravilu koje alat \textit{AutoHint} podr\v{z}ava odgovara jedan upariva\v{c}. Upariva\v{c}i su nazvani po broju pravila na koje se odnose i imaju imena
...
\subsection{AutoHint.cpp}
Ova jedica prevođenja predstavlja ulaznu ta\v{c}ku alata \textit{AutoHint}. U okviru nje, implementirane su klase \texttt{AutoHintConsumer} i \texttt{AutoHintAction} koje nasleđuju redom klase \texttt{ASTConsumer} i \texttt{ASTFrontendAction} (opisane u sekciji \ref{sec:interfejsi}). Osnovna uloga klase \texttt{AutoHintConsumer} jeste da obezbedi da se nad jedinicom prevođenja pokrenu odgovaraju\'{c}i upariva\v{c}i. To su upariva\v{c}i koji odgovaraju pravilima koje je korisnik zadao u okviru opcije komandne linije \texttt{--rules}. Parsiranje ove opcije i  pokretanje upariva\v{c}a nad apstraktnim sintaksnim stablom implementirano je u okviru metode \texttt{HandleTranslation\-Unit} klase \texttt{AutoHintConsumer}. Ova metoda se poziva tokom parsiranja na kraju izgradnje apstraktnog sintaksi\v{c}kog stabla za svaku jedinicu prevođenja nad kojom je pokrenut alat. Klasa \texttt{AutoHintAction} instancira objekat klase \texttt{AutoHintConsumer} u okviru metode \texttt{CreateASTConsumer}.
Na slici \ref{fig:uml1} prikazan je odnos klasa \texttt{AutoHintConsumer}, \texttt{AutoHintAction}, \texttt{ASTConsumer} i \texttt{ASTFrontendAction}. \par
U okviru jedinice prevođenja \texttt{Autohint.cpp} takođe je implementirana funkcija \texttt{main} alata \textit{AutoHint}. U okviru nje vr\v{s}i se parsiranje opcija komandne linije, kreira se instanca alata, kreiranoj instanci se pridru\v{z}uje objekat klase \texttt{AutoHint\-DiagnosticConsumer} i alat se pokre\'{c}e nad zadatom jedinicom prevođenja. 
...
\begin{tikzpicture}
  ...
  \end{class}
  \begin{class}[text width=5cm]{AutoHintConsumer}{-9,-5}
    \inherit{ASTConsumer}
    \operation{\texttt{HandleTranslationUnit()}}
  \end{class}
  ...
  \begin{class}[text width=5cm]{ASTFrontendAction}{1, 0}
    ...
    % \operation[0]{withdrawl(amount : Dollars)}
  \end{class}
  \begin{class}[text width=5cm]{AutoHintAction}{1, -5}
    \inherit{ASTFrontendAction}
    \operation{\texttt{CreateASTConsumer()}}
  \end{class}
    \draw[umlcd style dashed line ,->] (AutoHintAction) --node[above, sloped, black]{$<$ instantiate $>$} (AutoHintConsumer) ;
\end{tikzpicture}
  \caption{Odnos klasa \texttt{ASTConsumer}, \texttt{AutoHintConsumer}, \texttt{ASTFrontendAction} i \texttt{AutoHintAction}.}
  \label{fig:uml1}

\end{figure}
\subsection{AutoHintDiagnosticConsumer.cpp}
Vid obrade dijagnostike najrelevantniji za alat \textit{AutoHint} jeste njeno ispisivanje na standardni izlaz. U ovu 
svrhu \textit{AutoHint} koristi funkcionalnost postoje\'{c}e klase \texttt{TextDiagnosticPrinter} (opisane u sekciji \ref{sec:diagnostics}).
Pored ispisivanja poruka upozorenja i predloga za ispravljanje koda na standardni izlaz, alat \textit{AutoHint} konzumira dijagnostiku tako \v{s}to predlo\v{z}ene izmene koda primenjuje na izvorni fajl ukoliko je prosleđena opcija komandne linije \texttt{--apply-fix}.
Da bi se ovo postiglo u okviru alata \textit{AutoHint} implementirana je klasa \texttt{AutoHint\-DiagnosticConsumer}. Ova klasa nasleđuje
klasu \texttt{TextDiagnosticPrinter} i time zadr\v{z}ava funkcionalnost ispisivanja dijagnostike na standardni izlaz koja je implementirana u okviru nje. \par Klasa \texttt{AutoHintDiagnosticConsumer} predefini\v{s}e metod \texttt{HandleDiag\-nostic} i u okviru njega pored ispisivanja dijagnostike kreira objekat klase \texttt{Replace\-ment} za predlo\v{z}enu izmenu koda. Objekat klase \texttt{Replacement} \v{c}uva putanju do fajla (\texttt{std::\-string FilePath}), informacije o tome koje delove izvornog koda treba zameniti sa predlogom za ispravljanje koda i sam predlog (\texttt{Range ReplacementRange}, \texttt{std::string ReplacementText}).
U okviru metode \texttt{finish} svi kreirani objekti klase \texttt{Replacement} se pridru\v{z}uju objektu klase \texttt{Rewriter}.
Ova klasa omogu\'{c}ava primenjivanje predlo\v{z}enih izmena koda na izvorni fajl. Predlo\v{z}ene izmene koda primenjuju se pozivom metode \texttt{overwriteChangedFiles} klase \texttt{Rewriter}. Na slici \ref{fig:DiagnosticConsumer} prikazan je odnos klasa \texttt{DiagnosticConsumer}, \texttt{TextDiagnosticPrinter}, \texttt{AutoHintDiagnosticConsumer}, \texttt{Rewriter} i \texttt{Replacement}.
...
\begin{tikzpicture}
  ...
  \begin{class}[text width=7cm]{AutoHintDiagnosticConsumer}{-9, -6}
  ...
  \end{class}
    \draw[umlcd style dashed line ,->] (AutoHintDiagnosticConsumer) --node[above, sloped, black]{$<$ instantiate $>$} (Replacement) ;
    \draw[umlcd style dashed line ,->] (Rewriter) --node[above, sloped, black]{$<$ uses $>$} (Replacement) ;

\end{tikzpicture}
  \caption{Odnos klasa \texttt{DiagnosticConsumer}, \texttt{TextDiagnosticPrinter}, \texttt{AutoHintDiagnosticConsumer}, \texttt{Rewriter} i \texttt{Replacement}.}
  \label{fig:DiagnosticConsumer}

\end{figure}
\end{center}

\subsection{AutoHintHelper.cpp}

U okviru jedinice prevođenja \texttt{AutoHintHelper.cpp} implementirane su pomo\'{c}ne funkcije kori\v{s}\'{c}ene u okviru alata \textit{AutoHint}. U nastavku su ukratko opisane funkcije iz ove jedinice prevođenja.
\begin{itemize}
...
U okviru alata \textit{AutoHint} testirana je implementacija svakog od podr\v{z}anih pravila. Svaki test proverava implementaciju jednog pravila.
Testovi \texttt{auto-hint-A7-1-6\-.cpp}, \texttt{auto-hint-A7-2-3.cpp}, \texttt{auto-hint-A8-5-2.cpp} i \texttt{auto-hint-A8-5-3.cpp} nalaze se u okviru direktorijuma \texttt{autohint-test} i redom testiraju implementaciju pravila
...
Na listingu \ref{lst:test} prikazan je pojednostavljeni primer testa za pravilo \textbf{A7-1-6} u okviru alata \textit{AutoHint}. Test je pojednostavljen tako \v{s}to se u okviru njega nalazi samo jedan konstrukt koji nije u skladu sa pravilom \textbf{A7-1-6}. Pojednostavljeni test je kori\v{s}\'{c}en kako bi se demonstrirali svi bitni aspekata testiranja alata \textit{AutoHint} sa \v{s}to manje koda. \par Test
proverava da se pokretanjem alata \texttt{AutoHint} ispisuje adekvatno upozorenje zajedno sa predlogom za ispravljanje koda. U okviru \texttt{RUN} linije (linija 1) pokre\'{c}e se alat \textit{AutoHint} komandom \texttt{auto-hint --rules="A7\_1\_6" \%s 2>\&1 --}. Simbol \texttt{\%s}  \'{c}e prilikom pokretanja biti zamenjen putanjom do testa u kome se ova komanda nalazi (testa koji \textit{lit} pokre\'{c}e). \texttt{2>\&1} preusmerava standardni izlaz za gre\v{s}ke (STDERR) na standardni izlaz (STDOUT). Drugi deo komande \texttt{| FileCheck \%s} prosleđuje izlaz iz alata \textit{AutoHint} na standardni ulaz alata \textit{FileCheck} i pokre\'{c}e alat \textit{FileCheck} nad testom. Putanja do testa prosleđena je simbolom \texttt{\%s} \cite{LIT}.

 U okviru listinga \ref{lst:test} koriste se tri direktive alata \textit{FileCheck}, \texttt{CHECK: TEKST} (linija 6), \texttt{CHECK-NEXT: TEKST} (linije 8-11) i \texttt{CHECK-NOT: TEKST} (linije 5 i 12). Ove direktive slu\v{z}e da se izvr\v{s}i provera da li je alat ispisao upozorenje vezano za pravilo \textbf{A7-1-6}, predlog za ispravljanje koda i da se u okviru ispisa nije na\v{s}lo nijedno drugo upozorenje. U okviru direktive \texttt{CHECK} koriste se regularni izrazi za putanju na operativnom sistemu Linuks (eng.~\textit{Linux}), za po\v{c}etak i kraj linije. Redom, ovo su regularni izrazi \lstinline[style=custombash]{(/|/[a-zA-Z0-9_-]+)+}, \lstinline[style=custombash]{^} i \lstinline[style=custombash]{$}. Upotrebom regularnih izraza za po\v{c}etak i kraj linije test proverava da alat \textit{AutoHint} nije ispisao ni\v{s}ta nepredviđeno, odnosno da svaka linija po\v{c}inje i zavr\v{s}ava se tekstom navedenim između ta dva regularna izraza. Regualrni izraz za putanju na operativnom sistemu Linux koristi se kako test ne bi o\v{c}ekivao specifi\v{c}nu apsolutnu putanju u okviru ispisa alata \textit{AutoHint}. 

\begin{lstlisting}[style=customc, caption={Pojednostavljeni primer testa za pravilo \textbf{A7-1-6} u okviru alata \textit{AutoHint}.},label={lst:test}]
// RUN: auto-hint --rules="A7_1_6" %s 2>&1 -- | FileCheck %s

typedef unsigned long ulong;

// CHECK-NOT: {{.+}}
// CHECK: {{^((/|/[a-zA-Z0-9_-]+)+)}}/auto-hint-A7-1-6.cpp:3:23: warning: The typedef specifier shall not be used.{{$}}
...
U svrhu provere robusnosti i kvaliteta alata \textit{AutoHint}, izvr\v{s}ena je analiza nad delovima projekta \textit{Automotive Grade Linux (AGL)} \cite{AGL}. Projekat \textit{AGL} je izabran na osnovu svoje relevantnosti u automobilskoj industriji. \par
...
\par
Alat \textit{AutoHint} pokretan je nad dva podprojekta projekta \textit{AGL}. To su projekti \textit{re2c} i \textit{ninja}. Projekti su izabrani nasumi\v{c}no osim kriterijuma
da opseg projekta bude relativno mali. Analiza rezultata rada alata, odnosno upozorenja koje je alat \textit{AutoHint} prijavio, izvr\v{s}ena je ru\v{c}no od strane autora ovog rada. Kriterijum da projekat bude manjeg obima kori\v{s}\'{c}en je kako bi se smanjila verovatno\'{c}a gre\v{s}ke prilikom pomenute analize rezultata. Alat je pokretan skriptom koji rekurzivno 
obilazi direktorijume u okviru projekta i pokre\'{c}e alat \textit{AutoHint} nad  svim fajlovima sa ekstenzijom \texttt{.cpp} i \texttt{.cc}. Nad svakim fajlom, alat je pokrenut komandom \\
\texttt{./auto-hint  --rules="all"  putanja\_do\_fajla }

\subsection{Projekat \textit{re2c}}

Projekat \textit{re2c} nad kojim je pokretan alat \textit{AutoHint} nalazi se u okviru projekta \textit{AGL} na lokaciji \texttt{build/tmp/work/x86\_64-linux/re2c-native}. Projekat sadr\v{z}i 79 fajlova sa ekstenzom \texttt{.cc} \v{c}iji ukupan broj linija koda iznosi 21021. U okviru projekta
...
Tabela \ref{fig:re2c} prikazuje broj prijavljenih upozorenja za svako od pravila prilikom pokretanja alata \textit{AutoHint}.
...
Projekat \textit{ninja} nad kojim je pokretan alat \textit{AutoHint} nalazi se u okviru projekta \textit{AGL} na lokaciji \texttt{build/tmp/work/x86\_64-linux/ninja-native}. Projekat sadr\v{z}i 55 fajlova sa ekstenzom \texttt{.cc} \v{c}iji ukupan broj linija koda iznosi 12587. U okviru projekta
nalazi se 43 zaglavlja \v{c}iji ukupan broj linija koda iznosi 2291. \par
Tabela \ref{fig:ninja} prikazuje broj prijavljenih upozorenja za svako od pravila prilikom pokretanja alata \textit{AutoHint}.
Na osnovu rezultata sa slike prime\'{c}eno je da je odnos broja prijavljenih upozorenja po pravilu sli\v{c}an rezultatima prikazanim na tabeli \ref{fig:re2c} dobijenim pri pokretanju alata \textit{AutoHint} nad projektom \textit{re2c}. Na osnovu ovoga, zaklju\v{c}ci izvedeni tokom diskusije rezultata nad projektom \textit{re2c} va\v{z}e i za rezultate projekta \textit{ninja}. Skript za pokretanje alata \textit{AutoHint} nad projektom \textit{ninja} pokrenut je deset puta kako bi se izra\v{c}unalo prose\v{c}no vreme izvr\v{s}avanja alata, koje iznosi 18.002 sekunde.\par
\begin{table}[!h]
\begin{center}
\begin{tabular}{ |p{3cm}||p{2cm}|p{2cm}|p{2cm}|p{2cm}|  }
 \hline
...
koriste se alati za stati\v{c}ku analizu koda, koji bez pokretanja programa detektuju k\^{o}d koji nije napisan u skladu sa standardom. Alat \textit{AutoHint}, koji je razvijen u ovom radu, ispisuje upozorenja vezana za k\^{o}d koji nije napisan u skladu sa podskupom pravila iz standarda \textsc{autosar} C++14 koja se odnose na deklaracije u programskom jeziku C++14 i predla\v{z}e kako izmeniti k\^{o}d da bi bio u skladu sa standardom. \textit{AutoHint} podr\v{z}ava i opciju komandne linije kojom se predlo\v{z}ene izmene mogu primeniti na izvorni k\^{o}d. Alat je razvijen kori\v{s}\'{c}enjem biblioteka koje pru\v{z}a kompilatorska infrastruktura LLVM. Pravila standarda \textsc{autosar} C++14 podr\v{z}ana u okviru alata \textit{AutoHint} su \textbf{A8-5-3}, \textbf{A8-5-2}, \textbf{A7-1-6}, \textbf{A7-2-3}. Detaljnim testiranjem upotrebom alata \textit{lit} i \textit{FileCheck} iz kompilatorske infrastrukture LLVM obezbeđen je i visok kvalitet razvijenog alata. Analizom rezultata dobijenim pokretanjem alata \textit{AutoHint} nad podprojektima projekta \textit{AGL} zaklju\v{c}eno je da u okviru podprojekta \textit{re2c} i \textit{ninja} k\^{o}d nije napisan po standardu \textsc{autosar} C++14.
...
omogu\'{c}ila bi i podr\v{s}ku zna\v{c}ajno \v{s}ireg skupa pravila. U ovu svrhu u okviru alata \textit{AutoHint} mogao bi se integrisati stati\v{c}ki analizator kompilatora \textit{Clang} koji omogu\'{c}ava ovakav tip analize. Alat bi se mogao unaprediti i implementiranjem dodatnih opcija komandne linije koje bi omogu\'{c}ile korisniku ve\'{c}u
