...
Opisana je i biblioteka \texttt{LibASTMatchers} koja implementira jezik specijalne namene (eng.~\textit{domain specific language}) kojim se mogu prona\'{c}i i obraditi specifi\v{c}ne sintaksi\v{c}ke strukture iz apstraktnog sintaksi\v{c}kog stabla.
\par
...
koristi u kontekstu kreiranja i izvr\v{s}avanja akcija nad apstraktnim sintaksi\v{c}kim
stablom.
\par
...
\begin{enumerate}
  \item Prednji deo LLVM-a prevodi izvorni k\^od podr\v{z}anih jezika u LLVM međukod. U ovu fazu spadaju leksi\v{c}ka, sintaksi\v{c}ka i semanti\v{c}ka analiza
...
\begin{description}
  ...
  \item[Biblioteka \texttt{clangParse}]
        ...
        biblioteke \texttt{clangSema} kako bi ispitala semanti\v{c}ku validnost sintaksi\v{c}kih konstrukta (niza tokena) od kojih kreira \v{c}vorove apstraktnog sintaksi\v{c}kog stabla.
...
U računarstvu, \textbf{apstraktno sintaksi\v{c}ko stablo}, ili samo \textbf{sintaksi\v{c}ko stablo}, je drvoidna reprezentacija apstraktne sintaksi\v{c}ke strukture izvornog koda napisanog u programskom jeziku. Svaki čvor stabla predstavlja konstrukt koji se pojavljuje u izvornom kodu.
...
\begin{figure}[!ht]
  ...
  \caption{Apstraktno sintaksi\v{c}ko stablo za k\^{o}d iz listinga \ref{lst:label4} koje je generisano komandom: \texttt{clang -Xclang -ast-dump hello.cpp}.}
  \label{fig:ASTSlika}
\end{figure}

\v{C}vorovi od kojih je izgrađeno apstraktno sintaksi\v{c}ko stablo predstavljaju apstrakciju sintaksi\v{c}kih struktura iz samog jezika.
...
Da bi se izvršila analiza izvornog koda pomoću AST-posetioca potrebno je naslediti klasu 
 \texttt{RecursiveASTVisitor<Derived>} i predefinisati željene metode \texttt{Visit} u okviru nje. Ukoliko je metodama \texttt{Visit} pronađen nepravilan konstrukt izvornog koda mo\v{z}e se prijaviti upozorenje. Na listingu \ref{lst:visitor} prikazan je primer posetioca. Metod \texttt{VisitEnumDecl} (linija 6) \'c{e} biti pozvan nad svim objektima klase \texttt{EnumDecl} u apstraktnom sintaksi\v{c}kom stablu. U okviru nje, proverava se da li deklaracija koristi sintaksu \texttt{enum class} \footnote{Razlika između deklaracija nabrojivog tipa sa sintaksom \texttt{enum} i  \texttt{enum class} obja\v{s}njena je u okviru opisa pravila \textbf{A7-2-3} u sekciji \ref{sec:implementiranaPravila}.}, odnosno da li su konstante u okviru nabrojivih (\texttt{enum}) tipova definisane u svom unutra\v{s}njem opsegu. Ukoliko sintaksa \texttt{enum class} nije kori\v{s}\'{c}ena pri deklaraciji, lokacija ove deklaracije, ukoliko je validna, ispisuje se na standardni izlaz (linija 13).
...
U okviru kompilatora \textit{Clang} dijagnostika se prijavljuje upotrebom klase \texttt{Diagn\-osticsEngine}. Ova klasa slu\v{z}i 
...
\par
Nakon formulisanja izraza za uparivanje kreirani upariva\v{c} se pokre\'{c}e nad apstraktnim sintaksi\v{c}kim stablom. Ovo se posti\v{z}e pozivanjem metode \texttt{matchAST()} klase \texttt{MatchFinder}. 
...
\texttt{ASTConsumer} je apstraktna klasa koja omogu\'{c}ava izvr\v{s}avanje razli\v{c}itih akcija nad apstraktnim sintaksi\v{c}kim stablom nezavisno od toga kako je apstraktno sintaksi\v{c}ko stablo kreirano.
...
U okviru njega, nad asptraktnim sintaksi\v{c}kim stablom, mo\v{z}e se pozvati AST-upariva\v{c} koji \'{c}e izvr\v{s}iti obilazak i analizu apstraktnog sintaksi\v{c}kog stabla \cite{ASTConsumer}.
...
koja omogu\'{c}ava ispisivanje opcija koje se mogu zadati kompilatoru i \texttt{Preprocesso\-rFrontendAction} koja omogu\'{c}ava izvr\v{s}avanje akcija vezanih za pretprocesiranje izvornog koda. Međutim, naj\v{c}es\'{c}a upotreba ove klase vezana je za akcije koje se izvr\v{s}avaju nad apstraktnim sintaksi\v{c}kim stablom. U ovu svrhu koristi se apstraktna klasa \texttt{ASTFrontendAction} koja je direktna potklasa klase \texttt{FrontendAction}. \par
\texttt{ASTFrontendAction} predefini\v{s}e metod \texttt{executeAction} klase \texttt{FrontendAction}. U okviru ove metode pozivaju se funkcije za semanti\v{c}ku analizu i kreiranje apstraktnog sintaksi\v{c}kog stabla. Nad ovim apstraktnim sintaksi\v{c}kim stablom izvr\v{s}i\'{c}e se akcije implementirane u objektu klase \texttt{ASTConsumer} pridru\v{z}enom ovoj klasi.
...
Da bi se implementirala specifi\v{c}na akcija nad apstraktnim sintaksi\v{c}kim stablom, potrebno je implementirati klasu koja nasleđuje klasu \texttt{ASTFronendAction} i dodeliti joj objekat klase \texttt{ASTConsumer} koji implementira \v{z}eljenu akciju. Objekat se kreira i dodeljuje predefinisanjem metode \\ 
...
Na listingu \ref{lst:ASTAction} prikazana je implementacija klase \texttt{AutoFixAction} koja izvr\v{s}ava akcije nad apstraktnim sintaksi\v{c}kim stablom.
...
\begin{lstlisting}[style=customc,  caption={Implementacija klase \texttt{AutoFixAction}. Klasa \texttt{AutoFixConsumer} prikazana je na listingu \ref{lst:labelConsumer} i dostupna je kroz zaglavlje \texttt{AutoFixConsumer.h}.}, label=lst:ASTAction]
...
\begin{figure}[h!]
\begin{center}
\includegraphics[scale=0.51]{ASTFrontendAction.png}
\end{center}
...
zasnivaju se na upotrebi interfejsa ka apstraktnom sintaksi\v{c}kom stablu kompilatora \textit{Clang} ili kori\v{s}\'{c}enjem stati\v{c}kog analizatora kompilatora \textit{Clang} (eng.~\textit{Clang Static Analyzer}) za potrebe simboli\v{c}kog izvr\v{s}avanja programa. \par Alatke za stati\v{c}ku analizu mogu koristiti kombinaciju tehnika obrade apstraktnog sintaksi\v{c}kog stabla i simboli\v{c}kog izvr\v{s}avanja programa u zavisnosti od kompleksnosti analize koja je potrebna. Implementacija stati\v{c}ke analize obradom apstraktnog sintaksi\v{c}kog stabla je jeftinija po pitanju ra\v{c}unarskih resursa ali je ograni\v{c}ena informacijama dostupnim tokom kompilacije programa. \par 
 ...
 Kompilator \textit{Clang} pru\v{z}a vi\v{s}e infrastruktura za pisanje razli\v{c}itih vrsta softverskih alatki koje koriste sintaksi\v{c}ke i semanti\v{c}ke informacije o programu. U nastavku \'{c}e biti opisano nekoliko interfejsa koji se mogu koristiti u ovu svrhu zajedno sa njihovim prednostima i manama.
...
\begin{itemize}
  ...
  \item su potrebne apstrakcije visokog nivoa kao \v{s}to je iteriranje kroz apstraktno sintaksi\v{c}ko stablo sa kursorima \footnote{Kursori su instance strukture \texttt{CXCursor} i slu\v{z}e da ujedine razli\v{c}ite entitete u programu, na primer deklaracije i izraze, u apstakcije sa istim skupom operacija \cite{cursors}.} ili drugi detalji vezani za AST.
\end{itemize}
\texttt{LibClang} ne treba koristiti kada je potrebna puna kontrola nad apstraktnim sintaksi\v{c}kim stablom \cite{RightInterface}.

\item[Dodaci kompilatora \textit{Clang}] omogu\'{c}avaju izvr\v{s}avanje dodatnih akcija nad apstraktnim sintaksi\v{c}kim stablom tokom kompilacije programa. 
...
\item je potrebna potpuna kontrola nad apstraktnim sintaksi\v{c}kim stablom.
\end{itemize}
...
\begin{itemize}
  ...
  \item je potrebno imati punu kontrolu nad apstraktnim sintaksi\v{c}kim stablom kompilatora \textit{Clang}.
...
Kompilatorska infrastruktura LLVM sadr\v{z}i alatke koje se mogu koristiti u svrhu pisanja i pokretanja testova. U svrhu testiranja alata \textit{AutoFix}
...
Ukoliko se prilikom deklaracije nabrojivog tipa ne koristi sintaksa \texttt{enum class} mo\v{z}e do\'{c}i do ponovnog deklarisanja konstanti iz
